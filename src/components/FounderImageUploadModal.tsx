...
interface FounderImageUploadModalProps {
  onTrackAction?: (actionType: string, description: string) => void;
}

export const FounderImageUploadModal: React.FC<FounderImageUploadModalProps> = ({ onTrackAction }) => {
  const { imageUrl, updateImage, isModalOpen, closeModal } = useFounderImage();
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (file.size > 8 * 1024 * 1024) {
      setError('Image size should be under 8MB for optimal loading.');
      return;
    }
  ...
  const handleSave = () => {
    ...
    if (onTrackAction) {
      onTrackAction('founder_image_updated', previewUrl ? 'Profile image added to frame' : 'Profile image removed');
    }
    closeModal();
  };
  ...
  const handleRemove = () => {
    ...
    updateImage(null);
    if (onTrackAction) {
      onTrackAction('founder_image_removed', 'Profile image removed from frame');
    }
    closeModal();
  };
      ...
      >
        ...
        <div className="bg-[#0A2A66] p-5 text-white flex items-center justify-between border-b border-black">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base font-heading">Add Profile Image to Frame</h3>
              <p className="text-xs text-slate-300">Set portrait for {SITE_CONFIG.founderName}</p>
            </div>
          </div>
          <button
            onClick={closeModal}
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2.5 flex items-center justify-between">
              <span>Photo Frame Live Preview</span>
              {previewUrl && (
            ...
            <div className="flex items-center justify-center py-2">
              ...
              <div className="p-4 rounded-2xl bg-white border border-slate-200 flex flex-col items-center gap-2.5 shadow-sm">
                <div className="w-20 h-20 rounded-2xl border-2 border-[#0A2A66] bg-slate-100 flex items-center justify-center overflow-hidden shadow-inner relative">
                  {previewUrl ? (
                    <img 
                      src={previewUrl} 
                      alt="Founder preview" 
                    ...
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <span className="text-[9px] font-bold mt-1 tracking-wider uppercase">Empty Frame</span>
                    </div>
                  )}
                </div>
                <div className="text-center">
                  <p className="text-xs font-bold text-[#0A2A66]">{SITE_CONFIG.founderName}</p>
                  <p className="text-[10px] text-slate-500">{SITE_CONFIG.founderRole}</p>
                </div>
              </div>
            </div>
          </div>
